...
\author{Idrees Aziz \and Mahir Sadikhov \\
...
% ══════════════════════════════════════════════════════════════
\begin{abstract}
When large language models (LLMs) are asked to rate text quality on a numeric scale, they systematically avoid extreme scores---compressing their responses into a narrow interior band regardless of true quality variation. We present a controlled experiment measuring this \emph{score compression} effect across eleven language models spanning 3.8B to 228B parameters, using approximately 99{,}000 total evaluations based on 150 expert-authored articles sourced from The Conversation (theconversation.com). Each article is degraded along four independent quality axes (grammar, coherence, information, lexical sophistication) at five controlled intensity levels, producing a known ground-truth quality gradient. Across the full suite of eleven models, we find compression ratios ranging from 0.03 (Mistral~7B) to 0.69 (GPT-5~mini), with a positive correlation between model size and compression ratio (Pearson $r = 0.78$, $p = 0.012$). In a detailed interaction analysis of the two highest-performing frontier models, GPT-5~mini and Gemini~3~Flash, neither model ever assigns a score of 0 or 10 on a 0--10 scale---a boundary avoidance pattern shared by four additional models---despite calibrated simulations predicting hundreds of such scores. A three-way linear mixed-effects model confirms significant main effects of degradation level, axis, and model, along with all two- and three-way interactions. To address this universal bias, we evaluate five mitigation strategies: quantile normalization, expected-score rescaling, auxiliary regression, asymmetry correction, and contrastive anchor prompting. On a controlled nine-model comparison, auxiliary regression yields the largest gains in pairwise accuracy ($0.518 \to 0.702$) and expected-score rescaling improves rank correlation ($\rho: 0.458 \to 0.490$). Quantile normalization recovers the full scale (compression ratio $>$0.92) while perfectly preserving rank order. Contrastive anchor prompting produces mixed results. A post-hoc affine calibration experiment reduces RMSE by 10--25\%, confirming that the distortion is systematic and partially correctable. These findings demonstrate that LLM-based text evaluation is subject to a robust distributional bias, carrying practical implications for any application relying on LLM-generated quality ratings.
\end{abstract}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Multi-model evaluation.} We evaluate a comprehensive suite of eleven models spanning 3.8B to 228B parameters---including API-based frontier models (GPT-5~mini, Gemini~3~Flash) and open-weight models from Meta, Google, Microsoft, Alibaba, Mistral~AI, and MiniMax---to assess how compression varies with model scale and architecture. (We conduct detailed focal analyses on the highest-performing models to examine subtle interaction effects).
\end{enumerate}
...
\noindent where $\mathcal{C}$ is the set of corpus articles, $\mathcal{A}$ the set of degradation axes, $\mathcal{L}$ the set of intensity levels, and $R = 3$ the number of repetitions per condition, yielding 9{,}000 samples per model. Across all 11 models, this yields 99{,}000 total evaluations.
...
\begin{table}[t]
\centering
\caption{Models and configurations. All eleven models are evaluated at full factorial scale (9{,}000 samples each).}
\label{tab:models}
...
\begin{table}[H]
\centering
\caption{Descriptive statistics of score distributions for all eleven models, ordered by compression ratio (Table~\ref{tab:compression}). \#0s and \#10s denote the number of times a model assigned the minimum or maximum score. Three models returned fewer than 9{,}000 scores due to occasional unparseable responses: MiniMax~M2P1 ($n = 8{,}933$), Phi-4~mini ($n = 8{,}991$), and Qwen~2.5~7B ($n = 8{,}998$).}
\label{tab:dist}
\small
\begin{adjustbox}{max width=\columnwidth}
...
\begin{equation}
    \mathit{CR} = \frac{\bar{S}(\lambda = 0) - \bar{S}(\lambda = 0.8)}{10(1 - 0) - 10(1 - 0.8)} = \frac{\bar{S}(0) - \bar{S}(0.8)}{8}
    \label{eq:compression}
\end{equation}

A value of $\mathit{CR} = 1$ indicates no compression (exact agreement with the naive reference); $\mathit{CR} < 1$ indicates compression.
...
\begin{tabular}{@{}lrccc@{}}
\toprule
\textbf{Model} & \textbf{Params} & $\widehat{\mathit{CR}}$ & \textbf{95\% CI} & \textbf{Spearman} $\rho$ \\
\midrule
...
The average slope across axes ranges from $-6.46$ (GPT-5~mini) to $-0.28$ (Mistral~7B). Under the naive linear reference, a slope of $-10$ would be expected (each 0.1 increase in degradation level should decrease the score by 1 point). Only the top API models approach two-thirds of the ideal sensitivity; nine of eleven models achieve less than half. Figure~\ref{fig:forest} presents a forest plot of all slopes with 95\% confidence intervals.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.65\textheight,keepaspectratio]{G8_forest_plot.png}
\caption{Forest plot of regression slopes ($\beta_1$) with 95\% CIs for each axis$\times$model combination across all eleven models. All slopes are significantly negative except Mistral~7B lexical. Information deletion elicits the steepest response across models; coherence or lexical disruption the shallowest.}
\label{fig:forest}
\end{figure}
...
\caption{Cross-axis sensitivity comparison per model. Both models are most sensitive to information deletion; GPT-5~mini is least sensitive to coherence disruption, while Gemini~3~Flash is least sensitive to lexical degradation.}
\label{fig:crossaxis}
\end{figure}

\subsection{Non-linearity}

The dose-response curves (Figure~\ref{fig:doseresponse}) reveal visibly concave trajectories. A nested $F$-test comparing linear and quadratic regression models confirms significant quadratic terms for most axis$\times$model combinations (Table~\ref{tab:nonlin}). Among the top-performing models, all combinations are highly significant ($p < 10^{-20}$). Across all eleven models, 32 of 44 combinations show significant quadratic effects ($F > 3.84$, $p < 0.05$). All significant quadratic coefficients are positive, indicating concave dose-response curves: sensitivity is highest at low degradation levels and diminishes as quality approaches the floor. Models with very low CRs (Mistral~7B, Qwen~2.5~7B, Phi-4~mini) show weak or absent quadratic effects on several axes, reflecting their overall insensitivity to degradation.
...
Kendall's $\tau$ confirms monotonic relationships between degradation level and score across all models and axes (Table~\ref{tab:tau}). The pattern is consistent across the model landscape: information and grammar axes generally elicit the strongest monotonic responses, while coherence and lexical axes are weaker. Severely compressed models (Mistral~7B, Phi-4~mini) show notably weaker monotonicity, with $|\tau| < 0.20$ on several axes, indicating that these models barely distinguish between degradation levels.
...
The following two subsections focus on the two highest-performing frontier models (GPT-5~mini and Gemini~3~Flash) for detailed interaction analysis. These models were selected because they represent distinct API providers and both achieve CRs above 0.60 and Spearman $\rho > 0.70$ (Table~\ref{tab:compression}), making their comparison the most informative for understanding how two well-calibrated but architecturally distinct models differ in their scoring behavior. The mixed-effects specification requires exactly two model levels to estimate the three-way interaction, and the inter-model agreement metrics are inherently pairwise.
...
\begin{itemize}[nosep]
    ...
    \item The main effect of model (Gemini vs.\ GPT) is non-significant at the intercept ($\beta = +0.07$, $p = 0.25$), indicating comparable baseline scoring at $\lambda = 0$ for the reference axis. (The significant +1 point median offset reported in Section~\ref{sec:intermodel} emerges from the interaction terms: as degradation increases, the models diverge due to axis-specific slope differences.)
...
\begin{table}[t]
\centering
\caption{Inter-model agreement and systematic bias (API models).}
...
The compression results above raise a natural question: can the bias be corrected post hoc? We investigate using a calibration experiment designed as part of the initial protocol. The findings here generalize to the broader model set through the mitigation experiments in Section~\ref{sec:mitresults}. We investigate whether a learned calibration function can recover the lost scoring range. We split the 150 source articles into a training set (120 articles, 7{,}200 samples) and a held-out test set (30 articles, 1{,}800 samples) using an 80/20 article-level split, and fit three calibration functions on the training set: an affine transform $\hat{S} = aS_{\text{raw}} + b$, a four-parameter sigmoid $\hat{S} = d + c / (1 + e^{-a(S_{\text{raw}} - b)})$, and isotonic regression (a nonparametric monotonic mapping that serves as an upper bound on what any monotonic calibration can achieve). We evaluate on the held-out test set to avoid overfitting.

Table~\ref{tab:calibration} reports the results. Affine calibration reduces the root-mean-square error (RMSE) against the ideal score by 25\% for GPT-5~mini ($2.51 \to 1.88$) and 10\% for Gemini~3~Flash ($1.94 \to 1.74$). Strikingly, neither the sigmoid nor the isotonic regression---the best possible monotonic calibration---offers more than a marginal improvement over the simple two-parameter affine. This convergence demonstrates two things: (i)~the compression is well approximated as a linear distortion, and (ii)~the residual RMSE of $\sim$1.7--1.9 points represents an irreducible floor arising from the models' imperfect rank discrimination, which no monotonic transform can correct. Rank-order correlations (Kendall $\tau$, Spearman $\rho$) are unchanged by all calibrations, as expected for monotonic transforms.

\begin{table}[t]
\centering
\caption{Calibration recovery on held-out test set (30 articles, 1{,}800 samples). RMSE is computed against the ideal score $S^* = 10(1 - \lambda)$. Isotonic regression provides a nonparametric upper bound.}
...
Figure~\ref{fig:calibrecovery} visualizes the effect: the calibrated dose-response curves are pulled closer to the ideal line $S = 10(1 - \lambda)$, narrowing the shaded compression gap, though a residual error of 1.7--1.9 RMSE points remains even after calibration. This residual reflects the models' imperfect rank discrimination---their inability to linearly separate degradation levels---which no monotonic transform can correct.
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,height=0.45\textheight,keepaspectratio]{G13_category_effects.png}
...
Across the complete suite of eleven models evaluated in our 99{,}000-sample protocol, we analyze the relationship between model size and compression behavior. As reported in Table~\ref{tab:models}, models range from 3.8B parameters (Phi-4~mini) to 228B (MiniMax~M2P1), and all received the identical minimal prompt. 

\subsection{Compression Across Models}

As shown previously in Table~\ref{tab:compression}, compression ratios vary widely across the eleven models, from CR $= 0.026$ (Mistral~7B) to CR $= 0.685$ (GPT-5~mini). Smaller models tend to exhibit more extreme compression: models below 10B parameters all achieve CRs below 0.30, while models above 100B all reach 0.42 or higher.
...
The simplest mitigation: map the empirical score distribution to a target distribution via quantile matching~\citep{bolstad2003comparison}. We test two targets:
\begin{itemize}[nosep]
...
For models that return token-level log-probabilities over the score tokens (0--10), we replace the argmax score with the probability-weighted expected value, an approach successfully utilized by recent LLM-as-a-judge frameworks~\citep{liu2023geval}:
\begin{equation}
...
We propose an extension of expected-score rescaling that corrects for skewness in the score probability distribution:
\begin{equation}
...
A supervised post-hoc approach inspired by confidence calibration techniques~\citep{kadavath2022language}: train a regressor to predict the proxy ground truth from logprob-derived features. We extract four feature families:
\begin{itemize}[nosep]
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{spearman_rho_barplot.png}
...
Our results demonstrate that score compression is a robust, reproducible phenomenon across all eleven models tested. In the primary analysis, both GPT-5~mini and Gemini~3~Flash compress the effective scoring range to 62--69\% of the ideal, with neither model ever producing the minimum (0) or maximum (10) score on a 0--10 scale. Across all eleven models and approximately 99{,}000 total evaluations, compression ratios range from 0.03 to 0.69. This is not an artifact of the experimental design: calibrated simulations show that a simple linear scorer with equivalent noise would produce hundreds of extreme scores.
...
The models agree on the relative ordering of axis sensitivity: across all eleven models, information deletion consistently elicits the steepest response while coherence or lexical disruption elicits the shallowest (Table~\ref{tab:slopes}). The ordering information $>$ grammar holds consistently across the top-performing models, though the relative ranking of coherence and lexical varies: GPT-5~mini is least sensitive to coherence ($-4.78$) while Gemini~3~Flash is least sensitive to lexical degradation ($-5.26$). The magnitude varies dramatically (mean slopes from $-6.46$ to $-0.28$). The significant three-way interactions in the mixed model confirm that the models weight these dimensions differently.
...
Both API models achieve excellent intra-condition reliability (ICC $> 0.90$), and ten of eleven models achieve ICC $> 0.70$ (Table~\ref{tab:icc}). This confirms that compression is not a noise phenomenon---it is a stable, deterministic property of the scoring function.
...
We have demonstrated that score compression is a robust, reproducible phenomenon across a comprehensive suite of eleven language models spanning 3.8B to 228B parameters and multiple providers, confirmed through approximately 99{,}000 total evaluations. Across all evaluated models, compression ratios range from 0.03 to 0.69, correlating positively with model size ($r = 0.78$) but with notable exceptions. Focusing deeply on the highest-performing frontier models, both GPT-5~mini and Gemini~3~Flash systematically avoid the extremes of the 0--10 scale, compressing the effective range to 62--69\% of the ideal.
...
The findings in this paper have direct ethical implications for the increasingly common practice of using LLMs to evaluate human-generated text. As LLMs are deployed in high-stakes domains such as automated student essay grading, recruitment screening, and content moderation, systematic biases in their scoring distributions can translate directly into real-world harm.

First, the phenomenon of score compression means that naive, threshold-based decisions are inherently miscalibrated. If an institution sets an absolute cutoff---for instance, requiring a score of 9 out of 10 to receive a scholarship, or an 8 to pass a resume screen---the LLM's systematic boundary avoidance will result in the arbitrary failure of highly qualified candidates. Exceptional human effort is systematically undervalued (the ``ceiling effect''), while extremely poor submissions may avoid necessary remediation because their scores are artificially buoyed (the ``floor effect'').

Second, our multi-model analysis reveals that different language models possess systematically different scoring biases; for example, Gemini~3~Flash scores texts roughly one point higher than GPT-5~mini on the exact same scale. If institutions deploy different LLMs interchangeably, subjects evaluated by one model will be systematically advantaged or disadvantaged compared to those evaluated by another, creating arbitrary and opaque inequities.

Finally, while we offer post-hoc mitigation strategies to partially recover calibration, we emphasize that these techniques cannot recover discriminative signals that the underlying model failed to capture. We caution researchers, developers, and policymakers against treating LLM-generated scores as objective, interval-scaled ground truth. Uncalibrated LLM scores are at best ordinal signals, and utilizing them in automated decision-making requires rigorous, context-specific validation against domain-expert baselines to ensure fairness and accuracy. 
...
Bolstad, B.~M., Irizarry, R.~A., {\AA}strand, M., and Speed, T.~P. (2003).
...
\newblock In \emph{Proceedings of the 62nd Annual Meeting of the Association for Computational Linguistics}, pages 9440--9450.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.75\textheight,keepaspectratio]{G6_boxplots_grid.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{G7_violins.png}
