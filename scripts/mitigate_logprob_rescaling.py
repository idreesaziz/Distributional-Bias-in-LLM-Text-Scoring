...
def compute_skewness(row):
    ...
    if var <= 0: return 0.0
    std = np.sqrt(var)
    return float(np.dot((np.arange(11) - mean) ** 3, probs) / (std ** 3))
...
def tune_alpha(expected, skewness, levels, n_folds=5):
    ...
    for a in alphas:
        ...
        for _, val_idx in kf.split(expected):
            ...
            rmses.append(np.sqrt(np.mean((adj - target[val_idx]) ** 2)))
...
def main():
    ...
    for model in models:
        ...
        m_raw = eval_scores(raw_scores, levels); m_raw.update({"model": model, "method": "raw_argmax"})
        m_exp = eval_scores(expected, levels); m_exp.update({"model": model, "method": "expected_score"})
        m_adj = eval_scores(adjusted, levels); m_adj.update({"model": model, "method": f"asymmetry_fix_alpha={alpha:.1f}"})
        all_results.extend([m_raw, m_exp, m_adj])
        print(f"  {model:30s}  alpha={alpha:.1f}")

    cols = ["model", "method", "wasserstein", "compression_ratio",
            "spearman_rho", "spearman_p", "dose_response_slope", "pairwise_accuracy"]
    pd.DataFrame(all_results)[cols].to_csv(RES_DIR / "logprob_rescaling.csv", index=False)
    ...
    for idx, model in enumerate(models):
        ...
        sns.kdeplot(adj, ax=ax, label=f"Asym (alpha={alpha_m:.1f})", color="#1f77b4", linewidth=1.5)
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / "logprob_rescaling_kde.svg", format="svg", bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved \u2192 {FIG_DIR / 'logprob_rescaling_kde.svg'}")
    print("\nDone.")
